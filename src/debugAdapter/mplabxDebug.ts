...
export class MplabxDebugSession extends LoggingDebugSession {
	...
	protected async setBreakPointsRequest(response: DebugProtocol.SetBreakpointsResponse, args: DebugProtocol.SetBreakpointsArguments): Promise<void> {
		...
		const actualBreakpoints0 = clientLines.map(async l => {
			const { verified, line, id } = await this._runtime.setBreakpoint(path, BigInt(this.convertClientLineToDebugger(l)));
	...
	protected breakpointLocationsRequest(response: DebugProtocol.BreakpointLocationsResponse, args: DebugProtocol.BreakpointLocationsArguments, request?: DebugProtocol.Request): void {
		...
		// TODO: Figure out what this function wants
		// It looks like this function is trying to ask where breakpoints can be placed.
		// I don't think the MDB supports that.
		this._runtime.getBreakpoints().then(bps => {
			...
			if (stk){
				response.body = {
					stackFrames: stk.map((f, ix) => {
						...
						// TODO: Something may be needed when looking at the disassembly
						return sf;
					}),
	...
	// 	const targets = this._runtime.step(args.frameId);
	// 	response.body = {
